...
ファイルを削除 [Xcode/コマンド]　※実態も削除されるので注意
...
Why（修正理由）をコミットコメントに記入する  
```$ git commit -m "Fix typo" -a```  
...
リモート追跡ブランチのリポートリポジトリの情報を反映  
```$ git fetch```  

ローカルブランチにリポート追跡ブランチの内容を反映  
```$ git merge origin/ios/develop/current```  
...
マージ先のブランチをチェックアウト  
```$ git checkout ios/develop/dev1```  

ios/develop/current をios/develop/dev1にマージする  
```$ git merge ios/develop/current```  
...
ブランチの履歴を残すためにnon fast-forwardとする  
$``` git merge --no-ff ios/develop/current```  

### [パターン2] featureブランチを一度削除してから、再度ブランチを作成する

ローカルfeatureブランチの削除  
```$ git branch -d ios/feature/dev2```

リモートブランチの削除  
```$ git push --delete origin ios/feature/dev2```

#### コミットをせずに一時的に変更点を対比

保存する際にコメントを記載しておくと後からわかりやすい  
```$ git stash save ”(保存時のコメント)”```  
...
指定したスタッシュ名から復帰させて、そのstashを削除する  
...
Push時のデフォルト動作の設定を変更する(デフォルトをupstreamに設定)  
